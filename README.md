...
本系统专为**科研新手**设计，解决了选题阶段“想法太泛、情报滞后、资源错位”的痛点。
...
![系统架构图](./workflow_architecture.png)
...
### 2. 导入工作流 (DSL Import)
1.  下载本仓库 `workflow/` 目录下的 **`research-topic-copilot.yml`** 文件。
2.  进入 Dify 控制台，点击 **“创建应用” (Create App)** -> **“导入 DSL 文件” (Import DSL File)**。
3.  上传刚才下载的 `.yml` 文件。

### 3. 配置工具与模型
1.  进入工作流编排界面。
2.  在 **Gatekeeper** 和 **Convergence** 节点中，选择你的主流大模型。
3.  在 **Scout Agent** 节点中，填入你的 **Tavily API Key**。
